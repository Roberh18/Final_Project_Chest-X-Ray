...
NUM_EPOCHS_TRIAL = 25            # Number of epochs during hyperparameter tuning
NUMBER_OF_TRIALS = 100
NUM_EPOCHS = 100
DROPOUT = 0.4                   # Adjusted from 0.2
IMAGE_SIZE = 224                 # Adjusted from 224
FIRST_LAYER_SIZE = 3             # 1 for 'L' grayscale, 3 for 'RGB', defined in ChestXrayDataset class
CALC_DATASET_MEAN_STD = True    # 'True' if mean and std pixel values have not been pre calulated

# Model and Training Configuration
MODEL_TYPE = 'ResNet18_SEB_3'   # Custom models: 'DeeperCNN_6' 'DeeperCNN_8' 'DeeperCNN_8_SEB', Pre-trained models: 'ResNet50' 'ResNet18' 'ResNet18_2' 'ResNet18_SEB' 'ResNet18_SEB_2' 'DenseNet121'
...
# File paths
train_images_dir = "./train_images_filtered_80_10_10_Reduction-6"
val_images_dir = "./val_images_filtered_80_10_10_Reduction-6"
test_images_dir = "./test_images_filtered_80_10_10_Reduction-6"

#train_images_dir = "./train_images_filtered_70_15_15"
#val_images_dir = "./val_images_filtered_70_15_15"
#test_images_dir = "./test_images_filtered_70_15_15"
...
for dataset_name, images_dir in [('Training', train_images_dir),
    ...
    image_names = [
        ...
        if not img.startswith(".") and os.path.isfile(os.path.join(images_dir, img))
    ]
    ...
    total_labels = 0  # Total number of labels assigned
    ...
    # Now, print label counts sorted from most represented label to least
...
class ChestXrayDataset(Dataset):
    '''
    Description:
    - This class loads chest X-ray images from the specified root directory and stores image information, like their corresponding labels.
...
def add_se_to_resnet(model, reduction=16):
    for name, module in model.named_children():
        if isinstance(module, nn.Sequential):  # Residual block sequence
            for block_name, block in module.named_children():
                if hasattr(block, 'conv2'):  # Check for ResNet bottleneck or basic block
                    block.add_module("se", SEBlock(block.conv2.out_channels, reduction))
        elif isinstance(module, nn.Module):  # Recursively modify children
...
if CALC_DATASET_MEAN_STD:
    ...
    mean = [0.4964, 0.4964, 0.4964]
    std = [0.2474, 0.2474, 0.2474]
...
# Precompute class weights using label counts
...
print(f"Number of samples in sampler: {len(sample_weights)}")
...
train_loader = DataLoader(
    train_dataset, batch_size=BATCH_SIZE, sampler=sampler, num_workers=4, pin_memory=True, collate_fn=collate_fn
)
val_loader = DataLoader(
    val_dataset, batch_size=BATCH_SIZE, shuffle=False, num_workers=4, pin_memory=True, collate_fn=collate_fn
)
test_loader = DataLoader(
    test_dataset, batch_size=BATCH_SIZE, shuffle=False, num_workers=4, pin_memory=True, collate_fn=collate_fn
)
...
class DeeperCNN_6(nn.Module):
    """
    Custom CNN model with 6 convolutional layers.
    """
    def __init__(self):
        super(DeeperCNN_6, self).__init__()
        feature_size = IMAGE_SIZE // (2 ** 6)  # Adjusted for 6 pooling layers
        self.conv1 = nn.Conv2d(FIRST_LAYER_SIZE, 32, kernel_size=3, padding=1)
        ...
        self.fc1 = nn.Linear(1024 * feature_size * feature_size, 2048)
...
def get_model(model_type, freeze_layers):
    if model_type == 'DenseNet121':
    ...
    elif model_type == 'ResNet18_SEB':
        model = models.resnet18(weights='IMAGENET1K_V1')
        model = add_se_to_resnet(model, reduction=8)  # Add SE blocks with a reduction ratio of 16
        ...
        model = models.resnet18(weights='IMAGENET1K_V1')
        model = add_se_to_resnet(model, reduction=2)  
    ...
    elif model_type == 'ResNet18_SEB_3':
        model = models.resnet18(weights='IMAGENET1K_V1')
        model = add_se_to_resnet(model, reduction=4)  # Add SE blocks with a reduction ratio of 8
        num_features = model.fc.in_features
        if FREEZE_LAYERS:
             # Freeze earlier layers for feature extraction
            for name, param in model.named_parameters():
...
elif MODEL_TYPE in ['DenseNet121', 'ResNet50', 'ResNet50_1', 'ResNet50_2', 'ResNet18', 'ResNet18_1', 'ResNet18_2', 'ResNet18_SEB', 'ResNet18_SEB_1', 'ResNet18_SEB_2', 'ResNet18_SEB_3', 'ResNet18_3']:
    model = get_model(MODEL_TYPE, freeze_layers=FREEZE_LAYERS)
...
'''
# The criterion will be defined within the objective function during hyperparameter tuning.
...
if OPTIMIZER_TYPE == 'AdamW':
    ...
    optimizer = optims.Ranger(model.parameters(), lr=LEARNING_RATE, weight_decay=WEIGHT_DECAY)
else:
...
def train_model_with_scheduler(
    ...
    # Load the best model
    model.load_state_dict(torch.load(f"best_model-{MODEL_TYPE}.pth"))
...
def objective(trial):
    global train_loader, val_loader, test_loader, class_weights
    ...
    OPTIMIZER_TYPE = trial.suggest_categorical('optimizer', ['AdamW'])
    SCHEDULER_FACTOR = trial.suggest_float('scheduler_factor', 0.05, 0.6)
    SCHEDULER_PATIENCE = trial.suggest_int('scheduler_patience', 1, 3)
    MODEL_TYPE = trial.suggest_categorical('model_type', ['ResNet50_1', 'ResNet18'])
    ...
    # If Combined Loss is selected, tune the weights
    if LOSS_FUNCTION == 'Combined':
        FOCAL_WEIGHT = trial.suggest_float('focal_weight', 0.0, 1.0)
        DICE_WEIGHT = 1.0 - FOCAL_WEIGHT  # Ensure the weights sum to 1

    # Re-create data loaders with new batch size
    train_loader = DataLoader(
        train_dataset, batch_size=BATCH_SIZE, sampler=sampler, num_workers=4, pin_memory=True, collate_fn=collate_fn
    )
    val_loader = DataLoader(
    ...
    # Define loss function based on selection
    if LOSS_FUNCTION == 'BCE':
        criterion = nn.BCEWithLogitsLoss(pos_weight=class_weights)
    elif LOSS_FUNCTION == 'Combined':
        criterion = CombinedLoss(alpha=1, gamma=2, smooth=1e-5, logits=True, reduce=True, focal_weight=FOCAL_WEIGHT, dice_weight=DICE_WEIGHT, pos_weight=class_weights)
    else:
    ...
    for epoch in range(NUM_EPOCHS_TRIAL):
        ...
        # Composite Metric: ((Loss) + (1 - F1)) / 2
        composite_metric = (val_loss + (1 - val_f1))/2

        # Report the composite metric to Optuna
        trial.report(composite_metric, epoch)

        # Handle pruning based on the intermediate composite metric
        if trial.should_prune():
            raise optuna.exceptions.TrialPruned()

        # Update the best validation F1-score and loss if applicable
        if val_f1 > best_val_f1:
            best_val_f1 = val_f1
        ...
        # Print Progress
        print(f"Trial {trial.number}, Epoch {epoch+1}/{NUM_EPOCHS_TRIAL}, Validation Loss: {val_loss:.4f}, F1 Score: {val_f1:.4f}, Composite Metric: {composite_metric:.4f}")

        # Scheduler step based on validation loss
        scheduler.step(val_loss)

    return composite_metric  # Minimizing the composite metric



def train_with_best_hyperparameters(best_params):
    global BATCH_SIZE, LEARNING_RATE, DROPOUT, WEIGHT_DECAY, OPTIMIZER_TYPE, SCHEDULER_FACTOR, SCHEDULER_PATIENCE, MODEL_TYPE, FREEZE_LAYERS
    global train_loader, val_loader, test_loader, class_weights
    ...
    FREEZE_LAYERS = best_params['freeze_layers']  # Extract FREEZE_LAYERS
    LOSS_FUNCTION = best_params['loss_function']  # Extract Loss Function
    if LOSS_FUNCTION == 'Combined':
    ...
    # Re-initialize model with the best freeze_layers parameter
    model = get_model(MODEL_TYPE, freeze_layers=FREEZE_LAYERS)
    model.to(device)
    ...
    if LOSS_FUNCTION == 'BCE':
        ...
        criterion = criterion = CombinedLoss(alpha=1, gamma=2, smooth=1e-5, logits=True, reduce=True, focal_weight=FOCAL_WEIGHT, dice_weight=DICE_WEIGHT)
    else:
        raise ValueError(f"Invalid loss function: {LOSS_FUNCTION}")

    # Train the model with the best hyperparameters
    train_model_with_scheduler(model, train_loader, val_loader, criterion, optimizer, scheduler, epochs=NUM_EPOCHS)


# ================================
# 13. Run the Hyperparameter Optimization and Training Process
# ================================
...
if __name__ == "__main__":
    ...
    # Define study with a single objective: minimize the composite metric
    study = optuna.create_study(
        direction="minimize",
        study_name="Combined_loss_optimization_ResNet_2",
        storage='sqlite:///optuna_study.db',  # Optional: Persist study results
        load_if_exists=True
    )
    study.optimize(objective, n_trials=NUMBER_OF_TRIALS)

    # Retrieve the best hyperparameters based on the composite metric
    best_params = study.best_params

    # Optionally, visualize the optimization history
    fig = vis.plot_optimization_history(study)
    fig.write_image("optimization_history.png")  # Save the plot
    fig.show()
    

    # Retrain the model with the best hyperparameters
    train_with_best_hyperparameters(best_params)
